...
//title show hook.
function show_title_name() {
    the_title('<h1 class="product_title entry-title">', '</h1>');
}
add_action('woocommerce_before_shop_loop_item', 'show_title_name');
//title show hook closed.

//title remove hook.
remove_action('woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_title', 10);
//title remove hook closed.
...
//cart product single.
add_action('woocommerce_product_single_add_to_cart_text', 'add_to_cart_button');

function add_to_cart_button() {
    return __('Add To Cart', 'woocommerce');
}
//cart product single closed.
...
add_action('after_setup_theme', 'image_gallery');
...
//reviews remove in tabs closed.

//reviews display
function reviews_summary() {
    comments_template();
}

add_action('woocommerce_after_single_product_summary', 'reviews_summary');

//reviews display closed

//User Registration Coupon Create
function user_registration_coupon_create($user_id){
    ...
    update_user_meta($user_id, 'user_register_date', $date);
...
add_action('user_register','user_registration_coupon_create');
//User Registration Coupon Create Closed

//user login check new user and old
function user_coupon_createrd($user_login,$user) {
    ...
    $user_register_date = get_user_meta($user_id, 'user_register_date', TRUE);

    if ($current_date === $user_register_date) { 
        
    } else {
        update_user_meta($user_id, 'user_register_date', $current_date);
        $amount = '10'; // Amount
        $discount_type = 'fixed_cart'; // Type: fixed_cart, percent, fixed_product, percent_product
...
add_action('wp_login', 'user_coupon_createrd', 10, 2);
//user login check new user and old closed


//user logout coupon delete
function current_user_coupon_expiry(){
    global $current_user;
    get_currentuserinfo();
...
function current_user_apply_coupon(){
    ...
    $user_coupon_code = $current_user->user_login;
    $coupon_apply = WC()->cart->get_applied_coupons();
    foreach( $coupon_apply as $user_coupon=>$coupon_code ){
        if( $user_coupon_code !== $coupon_code ){
            WC()->cart->remove_coupon( $coupon_code );
            wc_clear_notices(); //message clear function
            wc_add_notice(__('Please Valid Coupon Code','woocommerce'),'error'); //message display
        }
    }
...
function discount_coupon_less_then_five_hundrad (){
    ...
    $coupon_name = $current_user->user_login;
    $cart_total = $woocommerce->cart->total;
    
    $current_user_coupon = WC()->cart->get_coupons();
    if(!empty($current_user_coupon)) {
    ...
    if($cart_total < 500)
    {
        ...
        $coupon_discount = WC()->cart->has_discount($coupon_name);
        if($coupon_discount){
        }else{
            WC()->cart->add_discount($coupon_name);
        }
    }
